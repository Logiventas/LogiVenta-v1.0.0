...
const TableUsers: React.FC<TableUsersProps> = ({ data }) => {
    ...
    const columnDefs: ColDef[] = [
        ...
        { headerName: "Descripción", field: "description", sortable: true, filter: false, resizable: true, minWidth: 150, flex: 1 },
        {
            headerName: "Editar",
            cellRenderer: (params) => (
                <Link to={`/userManagement/editProfile`} className="btn py-0 m-auto btn-primary">Editar</Link>
            ),
            width: 110,
        },
        {
            headerName: "Eliminar",
            cellRenderer: (params) => (
                <Link to={`/userManagement/deleteProfile/${params.data.id}`} className="btn py-0 btn-secondary">Eliminar</Link>
            ),
            width: 115,
        }
    ];
