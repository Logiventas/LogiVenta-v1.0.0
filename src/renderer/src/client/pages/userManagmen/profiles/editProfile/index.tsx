...
import { getPermissions } from './adapter/getProfile.adapter';


export const EditProfile = () => {
  const { idProfile } = useParams<any>();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        let profileData
        if(idProfile){
          profileData = await getPermissions(parseInt(idProfile, 10));
        }
  ...
  return (
    <div className='d-flex flex-column justify-content-around'>
      ...
      <TablePermission onPermissionsChange={handlePermissionsChange} permissions={profile.permissions} />
